Shader "Lensing/Microlensing_old"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Strength ("Strength", Range(0, 1)) = 0
        _PositionTex ("LensPositions", 2D) = "black" {}
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(VertexOutput i) : SV_Target
            {
                ...
                //     float2 xy = i.uv - (float2(u0, v0) + 0.5 * _ScaleFactor - float2(0.5, 0.5));
                //     float r2 = xy.x * xy.x + xy.y * xy.y;
                //     alpha += 0.005 * _Strength * xy / r2;
                // }

                float pixel_width = _PositionTex_TexelSize.x;
                int width = _PositionTex_TexelSize.z;
                for (int j = 0; j < width; j++)
                {
                    float2 uv_pixel = float2((0.5 + j) * pixel_width, 0.5);
                    float4 pos = SAMPLE_TEXTURE2D(_PositionTex, sampler_PositionTex, uv_pixel);
                    float2 xy = i.uv - pos.xy;
                    float r2 = xy.x * xy.x + xy.y * xy.y;
                    alpha += 0.005 * _Strength * xy / r2;
                }

                float4 color = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv - alpha);
                return color;
            }
